window.addEventListener("load", () => {
  ...
  function setDragListener() {
    draggables.forEach((draggable) => {
      ...
      draggable.addEventListener("dragend", () => {
        ...
        const ingridientOrigin = document.querySelector(
          `[data-ingridient='${copyIngridient}']`
        );
  ...
  function getDragAfterElement(burger, y) {
    const draggableElements = [
      ...burger.querySelectorAll(".draggable:not(.dragging)"),
    ];
...
const buttonSave = document.querySelector(".button__main-confirm");
const counterOrder = document.querySelector('.header__order-counter a');
...
if (localStorage.length === 0) {
  counterOrder.style = 'display: none';
} 
if (localStorage.length > 0) {
  counterOrder.textContent = localStorage.length - 1;
} 
...
buttonSave.addEventListener("click", () => {
  const buttonSubmit = document.querySelector(".button__main-submit");
  const burger = document.querySelector(".burger"),
    burgerIngredients = burger.querySelectorAll(".draggable");
  ...
  buttonSubmit.addEventListener("click", () => {
    ...
    burgerIngredients.forEach((item) => {
      priceSum.push(item.dataset.price);
    });
    ...
    if (localStorage.length == 0) {
      counterOrder.style = 'display: block';
      counterOrder.textContent = 1;
    } else if (localStorage.length > 1) {
      counterOrder.textContent = counter;
    }

    let result = totalSum += sum;
    console.log(totalSum);
    localStorage.setItem('Total', result);
    localStorage.setItem(`Burger #${counter}`, `Burger #${counter} - ${sum}$`);

    clearTimeout(timer);

    let added = document.createElement("div");
    added.classList.add("added");
    added.innerHTML = "Added to your order";
    popUpModal.innerHTML = "";
    popUpModal.append(added);
    timer = setTimeout(() => {
      if (popUpModal.querySelector(".added")) {
        popUpModal.querySelector(".added").remove();
      }
    }, 2000);
  });
});
